...
@NamedQueries({
        ...
        //"SELECT r.id, r.name, r.registered, m.description, COUNT(v) FROM Restaurant r JOIN r.meals m ON m.date=:date LEFT JOIN r.voices v ON v.dateVoice=:date GROUP BY r.id, m.description"
        //"SELECT r.id, r.name, r.registered, m.description, m.price, m.date FROM Restaurant r LEFT JOIN r.meals m ON m.date=:date"
        //"SELECT DISTINCT r FROM Restaurant r left join fetch r.meals m where m.date=:date"
})
@Entity
